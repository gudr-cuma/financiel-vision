          ...
          {items.map((item, i) => {
            if (item.type === 'section') return (
              <tr key={i} style={{ background: '#B1DCE2' }}>
                <td colSpan={5} style={{ padding: '7px 10px', fontWeight: 700, fontSize: '12px', color: '#1A202C', letterSpacing: '0.04em' }}>
                  {item.label}
                </td>
              </tr>
            );
            ...
            if (item.type === 'subsection') return (
              <tr key={i} style={{ background: '#E3F2F5' }}>
                <td colSpan={5} style={{ padding: '5px 10px 5px 20px', fontWeight: 600, fontSize: '12px', color: '#4A5568', fontStyle: 'italic' }}>
                  {item.label}
                </td>
              </tr>
            );

            if (item.type === 'sublabel') return (
              <tr key={i}>
                <td colSpan={5} style={{ padding: '4px 10px 2px 28px', fontSize: '12px', color: '#718096', fontStyle: 'italic' }}>
                  {item.label}
                </td>
              </tr>
            );
            ...
            if (item.type === 'total') return (
              <tr key={i} style={{ background: '#E8F5E0', borderTop: '1px solid #C6E38A' }}>
                ...
                <td style={{ padding: '7px 10px', textAlign: 'right', fontWeight: 700, fontFamily: 'monospace' }}>{fmtEur(item.brut)}</td>
                <td style={{ padding: '7px 10px', textAlign: 'right', fontWeight: 700, fontFamily: 'monospace' }}>{fmtEur(item.amort)}</td>
            ...
            if (item.type === 'grandtotal') return (
              <tr key={i} style={{ background: '#1A202C' }}>
                ...
                <td style={{ padding: '9px 10px', textAlign: 'right', fontWeight: 700, color: '#FFFFFF', fontFamily: 'monospace' }}>{fmtEur(item.brut)}</td>
                <td style={{ padding: '9px 10px', textAlign: 'right', fontWeight: 700, color: '#FFFFFF', fontFamily: 'monospace' }}>{fmtEur(item.amort)}</td>
            ...
            if (item.type === 'footnote') return (
              <tr key={i}>
                <td style={{ padding: '3px 10px', fontSize: '11px', color: '#A0AEC0', fontStyle: 'italic' }}>{item.label}</td>
                <td colSpan={2} />
                <td style={{ padding: '3px 10px', textAlign: 'right', fontSize: '11px', color: '#A0AEC0', fontFamily: 'monospace' }}>{fmtEur(item.netN)}</td>
                <td style={{ padding: '3px 10px', textAlign: 'right', fontSize: '11px', color: '#A0AEC0', fontFamily: 'monospace' }}>{fmtEur(item.netN1)}</td>
              </tr>
            );
